...
def plot_texts(filename, label_class, max_rank, text_size, regulate_case=False, secondary_attr=None, force_points=False) -> str:
	...
	rank_idx, name_idx, type_idx, secd_idx, lat_idx, lon_idx = None, None, None, None, None, None
	...
	for record, shape in zip(sf.records(), sf.shapes()):
		...
		if rank <= max_rank:
			...
			result += '\t<text class="label-{} label-{}" x="{:.03f}" y="{:.03f}">{}</text>\n'.format(
			...
			if force_points or (type_idx is not None and record[type_idx] in ['mountain', 'depression', 'pole', 'waterfall']): #add circles to the ones that need markers
				result += '\t<circle cx="{:.03f}" cy="{:.03f}" r="{:.03f}" />\n'.format(180+x, 90-y, math.sqrt(text_size-rank+1)*0.15)
	return result
